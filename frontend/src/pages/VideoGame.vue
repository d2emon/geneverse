...
function ToggleSanity()
{
	if (!l('toggle').checked) {
	  Sanity=0;
	  l('sanity').style.color='#f00';
	} else {
	  Sanity=1;
	  l('sanity').style.color='#fff';
	}
}
...
function Generate()
{
	...
	// else str=C(C(Genres).name)+' where you '+C(C(Genres).actions)+' '+C(C(Genres).things)+' '+C(C(Genres).goals)+'.';
	  ...
	  str=C(allNames)+' where you '+C(allActions)+' '+C(allThings)+' '+C(allGoals)+'.';//better

	str=str.Cap()

	l('text').innerHTML=('<a href="javascript:Generate();" class="'+(Sanity?'sane':'insane')+'">'+str+'</a>');
	l('log').innerHTML='<div>&bull; '+str+'</div>'+l('log').innerHTML;
}
